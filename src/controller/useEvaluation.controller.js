...
useEvaluationRouter.get('/share/evaluation', async (req, res) => {
  // Este metodo se llamadra cuando el usuario (profesor) precione el boton compartir evalucion, generara un token con los datos de la evaluacion
})

useEvaluationRouter.post('/join/evaluation', async (req, res) => {
  const user = req.user // datos de usuario
  // Este metodo se llamadra cuando el usuario (alumno) entre a la pantalla unirme a evaluacion, donde ingresara el token compartido
